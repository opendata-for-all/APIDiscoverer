...
public class RESTClient {
	...
	public static Response send(APIRequest apiRequest) throws UnirestException {
		...
		case GET:
			rest = Unirest.get(apiRequest.getUrl()).header("Accept", "application/json").header("Content-type",
					"application/json");
			if(!apiRequest.getBody().equals(""))
			...
			break;
		case DELETE:
			rest = Unirest.delete(apiRequest.getUrl()).header("Accept", "application/json").header("Content-type",
					"application/json");
			if(!apiRequest.getBody().equals(""))
			((HttpRequestWithBody) rest).body(apiRequest.getBody());
			httResponse = rest.asString();
		...
		case POST:
			rest = Unirest.post(apiRequest.getUrl()).header("Accept", "application/json").header("Content-type",
					"application/json");
			((HttpRequestWithBody) rest).body(apiRequest.getBody());
			...
			response.setStatusText(httResponse.getStatusText());
		case PUT:
			rest = Unirest.put(apiRequest.getUrl()).header("Accept", "application/json").header("Content-type",
					"application/json");
			((HttpRequestWithBody) rest).body(apiRequest.getBody());
			httResponse = rest.asString();
			response.setBody(httResponse.getBody());
