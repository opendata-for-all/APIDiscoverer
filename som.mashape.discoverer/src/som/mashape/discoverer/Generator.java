...
public class Generator {
	...
	public static JsonObject includeMetadData(JsonObject openAPIJson,JsonObject metaDataJson){
		JsonObject infoJson = openAPIJson.get("info").getAsJsonObject();
		JsonObject infoAdditionalData = metaDataJson.get("info").getAsJsonObject();
		Set<Entry<String, JsonElement>> set = infoAdditionalData.entrySet();
		for(Entry<String, JsonElement> entry : set)
			infoJson.add(entry.getKey(), entry.getValue());
		return openAPIJson;
	}
	...
	private static void generateInfo(Info info, JsonObject jsonInfo) {
		...
		if(info.getContact()!= null){
			...
			generateContact(info.getContact(),jsonContact);}
		//TODO license
		jsonInfo.addProperty("version", info.getVersion());
		
		
	}
			private static void generateContact(Contact contact, JsonObject jsonContact) {
		if(contact.getName()!= null){
			jsonContact.addProperty("name", contact.getName());
		}
		if(contact.getUrl()!= null){
			jsonContact.addProperty("url", contact.getUrl());
		}
		if(contact.getEmail()!= null){
			jsonContact.addProperty("email", contact.getEmail());
		}
